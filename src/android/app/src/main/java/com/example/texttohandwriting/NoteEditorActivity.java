...
import com.example.texttohandwriting.security.SecureKeyManager;
...
public class NoteEditorActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Initialize secure key manager for API keys
        secureKeyManager = new SecureKeyManager(this);
     ...
     * Uses secure key management for API key storage and retrieval.
     */
    private void formatTextWithAI() {
        ...
        String apiKey = secureKeyManager.getOpenAIKey();
        if (apiKey == null || !secureKeyManager.isValidOpenAIKey(apiKey)) {
        ...
        client.getService().formatText("Bearer " + apiKey, request)
    ...
    private void handleApiError(Response<OpenAIResponse> response) {
        ...
        // If response indicates authentication error, prompt for API key
    ...
    /**
     * Shows a dialog for users to securely set up their OpenAI API key.
     * The key is encrypted and stored using Android Keystore for security.
     */
    private void showApiKeySetupDialog() {
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("OpenAI API Key Setup");
        builder.setMessage("Please enter your OpenAI API key to enable AI text formatting. " +
                "Your key will be securely encrypted and stored locally.");
        ...
        builder.setPositiveButton("Save", (dialog, which) -> {
            ...
            if (secureKeyManager.isValidOpenAIKey(apiKey)) {
                boolean saved = secureKeyManager.storeOpenAIKey(apiKey);
                if (saved) {
                    Toast.makeText(this, "API key saved securely", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(this, "Failed to save API key", Toast.LENGTH_SHORT).show();
                }
            } else {
                Toast.makeText(this, "Invalid API key format. Please check and try again.", 
        ...
        // Add option to clear existing key
        if (secureKeyManager.hasOpenAIKey()) {
            builder.setNeutralButton("Clear Key", (dialog, which) -> {
                boolean cleared = secureKeyManager.deleteOpenAIKey();
                if (cleared) {
